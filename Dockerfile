...
RUN tar -xvf latest-ru_RU.tar.gz -C /var/www/html/
...
RUN tar zxvf prometheus-*.linux-amd64.tar.gz
RUN rm prometheus-*.linux-amd64.tar.gz
RUN cp -r ./prometheus-*.linux-amd64/prometheus ./prometheus-*.linux-amd64/promtool /usr/local/bin/
RUN cp -r ./prometheus-*.linux-amd64/console_libraries ./prometheus-*.linux-amd64/consoles ./prometheus-*.linux-amd64/prometheus.yml /etc/prometheus
...
RUN tar zxvf alertmanager-*.linux-amd64.tar.gz
RUN rm alertmanager-*.linux-amd64.tar.gz
RUN cp ./alertmanager-*.linux-amd64/alertmanager ./alertmanager-*.linux-amd64/amtool /usr/local/bin/
RUN cp ./alertmanager-*.linux-amd64/alertmanager.yml /etc/alertmanager
...
RUN tar zxvf node_exporter-*.linux-amd64.tar.gz
RUN rm node_exporter-*.linux-amd64.tar.gz
RUN cp ./node_exporter-*.linux-amd64/node_exporter /usr/local/bin/
